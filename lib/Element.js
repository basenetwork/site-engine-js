...
function Element(path, host) {
    this.host = host;
    this.path = path;
    this.name = decodeURIComponent(path.match(/([^\/]*)\/$/)[1]);
    this.key = _key++;
}
...
ex(Element.prototype, {
    ...
    type: function() {
        if(this.isRoot()) return "homepage";
    ...
    children: function() {
        ...
        var req = {
            ...
            sse:     Application.element() === this
        };
        baseAPI.requestData(req, function(err, packs) {
            var isSSE = err === "sse-updates";
            if(err && !isSSE) return {error: err};
            if(isSSE && this._children) {
                // remove added elements (elements without hash)
                this._children = this._children.filter(function(e){
                    return e.hash && e;
                });
            }
            this._children = packs.map(function(pack){
                return _newElement(this.path + encodeURIComponent(pack.pos), this.host, pack);
            }.bind(this)).concat(this._children);
            this._refresh();
        }.bind(this));
    ...
    get: function(name) {
        ...
        var parts = name.split('|'), val;
        if(parts.length > 1) {
            while(parts.length) {
                if(val = this.get(parts.shift())) return val;
            }
            return null;
        }
    ...
    setName: function(name) {
        this.name = name;
        this.path = (this.parent() || {path: ""}).path + encodeURIComponent(name) + "/";
        return this;
    },

    newChildElement: function(name) {
        // new element. (empty name is possible)
        var element = new Element(this.path + encodeURIComponent(name||"") + "/", this.host);
        var streamInf = this.getChildStreamInfo();
