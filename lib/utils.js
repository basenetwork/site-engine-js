...
String.prototype.toCSSFormat = function() {
    return this.replace(/[A-Z][^A-Z]+/g, function(s){ return "-"+s.toLowerCase() }).replace(/^-/,"");
};
...
function $component(name) {
    for(var args = [], i=1, n=arguments.length; i<n; i++) args.push(arguments[i]);
    var cls = $class.apply(this, args);
    name.split(',').forEach(function(type){
        Application.components[type] = cls;
    });
    return cls;
}
